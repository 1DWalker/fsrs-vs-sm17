...
class LSTM(torch.nn.Module):
    ...
    def __init__(
        self, state_dict=None, input_mean=None, input_std=None
    ):
        super().__init__()
    ...
    def batch_process(
        ...
        w_nh = w_lnh[seq_lens - 1, torch.arange(n)]
        s_nh = s_lnh[seq_lens - 1, torch.arange(n)]
        d_nh = d_lnh[seq_lens - 1, torch.arange(n)]
...
def run_lstm(revlogs):
    ...
    for i in tqdm(range(len(revlogs))):
        ...
        if not np.isnan(row["next_index"]):
            ...
            sequence = torch.cat([x[..., :1], const, x[..., 1:]], dim=-1)
